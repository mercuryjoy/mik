...
@section('htmlheader_title')
    @if(in_array($type, ['red_envelope', 'point', 'shop_owner']))
        新建{{\App\Activity::$typeDisplays[$type]}}活动
    @else
        {{\App\Activity::$typeDisplays[$type]}}
    @endif
@endsection
...
@section('breadcrumb')
    ...
    @if(in_array($type, ['red_envelope', 'point', 'shop_owner']))
        <li class="active">新建{{\App\Activity::$typeDisplays[$type]}}活动</li>
    @else
        <li class="active">{{\App\Activity::$typeDisplays[$type]}}</li>
    @endif

@endsection

@section('main-content')
    {!! Form::model(new App\Activity, ['route' => ['admin.activities.store'], 'method' => 'POST']) !!}
    ...
    <div class='row'>
        <div class='col-md-6'>
            <div class="box">
                ...
                @else
                    <h3 class="box-title">红包信息</h3>
                @endif
                </div>
                <div class="box-body">
                    {!! Form::text('type', old('type', $type), ['class' => 'hidden']) !!}
                    {!! Form::text('action_zone', old('action_zone', 'part'), ['class' => 'hidden']) !!}
                    ...
                    <div class="form-group{{ $errors->has('shop_ids') ? ' has-error' : '' }}">
                        ...
                        {!! Form::text('shop_ids', old('shop_ids', ''), ['class' => 'form-control hidden', 'id' => 'shop_ids']) !!}
                        {!! Form::textarea('shop_names', old('shops_names', ''), ['class' => 'form-control', 'id' => 'shop_names', 'readonly' => 'readonly', 'placeholder' => '请选择终端']) !!}
                    ...
                    @if($type == 'point')
                        <div class="form-group{{ $errors->has('point') ? ' has-error' : '' }}">
                            {!! Form::label('point', '积分') !!}
                            {!! Form::number('point', old('point'), ['class' => 'form-control', 'min' => 0, 'max' => 100, 'step' => 1]) !!}
                    ...
                    @elseif($type == 'shop_owner')
                        <div class="form-group{{ $errors->has('money') ? ' has-error' : '' }}">
                            ...
                            {!! Form::number('money', old('money'), ['class' => 'form-control', 'min' => 0, 'max' => 100, 'step' => 0.01]) !!}
                    ...
                    @elseif ($type == 'send_red_envelope')
                        <div class="form-group{{ $errors->has('user_ids') ? ' has-error' : '' }}">
                            {!! Form::label('user_ids', '服务员') !!}
                            <button type="button" data-toggle="modal" data-target="#usersModal" class="btn btn-block btn-primary btn-sm"><i class="fa fa-plus-square"></i> 选择服务员</button>
                            {!! Form::text('user_ids', old('user_ids', ''), ['class' => 'form-control hidden', 'id' => 'user_ids']) !!}
                            {!! Form::textarea('user_names', old('user_names', ''), ['class' => 'form-control', 'id' => 'user_names', 'readonly' => 'readonly', 'placeholder' => '请选择服务员']) !!}
                            {!! $errors->first('user_ids', '<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>   :message</label>') !!}
                        </div>

                        <div class="form-group{{ $errors->has('point_amount') ? ' has-error' : '' }}">
                            {!! Form::label('point_amount', '积分(积分数量需在1~1000之间)') !!}
                            {!! Form::number('point_amount', old('point_amount'), ['class' => 'form-control', 'min' => 0, 'max' => 1000, 'step' => 1]) !!}
                            {!! $errors->first('point_amount', '<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>   :message</label>') !!}
                        </div>

                        <div class="form-group{{ $errors->has('money_amount') ? ' has-error' : '' }}">
                            {!! Form::label('money_amount', '现金金额(需在0.01~500之间)') !!}
                            {!! Form::number('money_amount', old('money_amount'), ['class' => 'form-control', 'min' => 0, 'max' => 500, 'step' => 0.01]) !!}
                            {!! $errors->first('money_amount', '<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>   :message</label>') !!}
                        </div>
                    @endif
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            {!! Form::text('daterange', old('daterange', request()->daterange), ['class' => 'daterange form-control']) !!}
                        </div>
                    </div>
                    @endif
    ...
    <div class="modal fade" id="usersModal" tabindex="-1" role="dialog" aria-labelledby="selectUserModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    选择服务员
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default pull-left" data-dismiss="modal">取消</button>
                    <button type="button" id="selectUserBtn" class="btn btn-primary btn-ok" data-dismiss="modal">确认</button>
...
@section('javascript')
    ...
    $("#selectUserBtn").click(function () {
        ...
        if (userIdArr.length > 0) {
            userIdAll = userIdArr.join(',');
            userNameAll = userNameArr.join('，');
        }

        $('#user_ids').val(userIdAll);
        $('#user_names').val(userNameAll);
    });
